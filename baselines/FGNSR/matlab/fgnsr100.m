function [X, K, fval, mmx_err, fval_hist, diag_hist, Tracking] = fgnsr100(M, r,varargin)
...
%        break;
    ...
    duration = duration + toc(iter_tic);
    Tracking.time(end+1) = duration;
    tmp = whos;
    Tracking.bytes(end+1) = sum([tmp.bytes]);
    Tracking.nnz(end+1) = nnz(X);
    if ip.Results.debug
        [~, kk] = sort(diag(X), 'descend');
        Tracking.rel_approx_error(end+1) = compute_approx_error(M, kk(1:rold));
    end
end
