function [ theta, B , Tracking] = SPACL_pruned( A, K, prune )
%
...
% ==============================
% Modified by TriNguyen
% ==============================
fprintf('Before truncating: %d \n', size(V, 1));
[~, III] = truncateData(V, 0.99);
% ==============================
% END 
% ==============================
...
% Modified by TriNguyen
% ==============================
I_outlier = unique([I_outlier(:); III(:)]);
fprintf('After truncating, number of outliers: %d \n', numel(I_outlier));
% ==============================
% End modifying
% ==============================


Tracking.I_outlier = I_outlier;
% Find pure nodes set that has pure nodes of all communities
[ pure ] = SPA( V, K, I_outlier );
Tracking.lambdaHat = pure;
% Estimate theta and B
theta = V / V(pure,:);
disp(V(pure, :))
tre = 1e-12;
theta(theta<tre) = 0;
% fprintf('AFTER: norm(theta): %f \n', norm(theta, 'fro'));
% fprintf('AFTER: neg %f \n', sum(theta<0, 'all'));
% fprintf('AFTER: mean %f \n', mean(theta(:)));
%
% fprintf('size(theta): %s \n', num2str(size(theta)));
% fprintf('norm(theta): %f \n', norm(theta, 'fro'));
...
% We dont need B in this experiment
