...
void multipleMatrixVector(
    Eigen::SparseMatrix<double, Eigen::RowMajor>& tXT,
...
double computePhiC(Eigen::SparseMatrix<double, Eigen::RowMajor> tCT, 
        double epsilon);
void fwMain(double *Xsr, mwIndex *Xjcs, mwIndex *Xirs, double *sr, mwIndex *jcs, mwIndex *irs, mwSize m, 
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    // Verify the matrix X is infact sparse
    ...
    fwMain(Xsr,Xjcs,Xirs,sr,jcs,irs,nrows,ncols,numThreads,estimatedN,maxIters,iterStart,
...
void fwMain(double *Xsr, mwIndex *Xjcs, mwIndex *Xirs, double *sr, mwIndex *jcs, mwIndex *irs, mwSize m, 
    ...
    if (verbose) {
        ...
        mexPrintf("NNZ of X: %d \n", Xjcs[n]);
    }
    mwSize i,j;
    size_t row, col;
    double val;
    std::vector<T> tripletList;
    tripletList.reserve((size_t)Xjcs[n]);
    /* Construct Sparse matrix X in Eigen */
    for (i = 1; i < n+1; ++i) {
        for (j = (size_t)Xjcs[i-1]; j < size_t(Xjcs[i]); ++j) {
            row = i-1;
            col = Xirs[j];
            val = Xsr[j];
    ...
    Eigen::SparseMatrix<double, Eigen::RowMajor> tXT(n,m);
    Eigen::SparseMatrix<double, Eigen::RowMajor> tmp1(n,m);
    Eigen::SparseMatrix<double, Eigen::RowMajor> tmp2(n,m);
    tXT.setFromTriplets(tripletList.begin(), tripletList.end());
    /* End construction */
    ...
    for (size_t iter = iterStart; iter <= maxIters; ++iter) {
        ...
        tCT.setFromTriplets(tripletList.begin(), tripletList.end());
        tmp1 = tCT*tXT;
        fittingError = (tXT - tmp1).squaredNorm();
